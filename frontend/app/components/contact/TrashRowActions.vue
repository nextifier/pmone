    ...
    <DialogResponsive v-model:open="restoreDialogOpen">
      <template #default>
        <div class="px-4 pb-10 md:px-6 md:py-5">
          <div class="text-primary text-lg font-semibold tracking-tight">Restore contact?</div>
      ...
      <template #default>
        <div class="px-4 pb-10 md:px-6 md:py-5">
          ...
          <p class="text-body mt-1.5 text-sm tracking-tight">
            This action can't be undone. This will permanently delete this contact.
          </p>
          <div class="mt-3 flex justify-end gap-2">
            <button
              class="border-border hover:bg-muted rounded-lg border px-4 py-2 text-sm font-medium tracking-tight active:scale-98"
...
const handleRestore = async () => {
  ...
  try {
    ...
    const response = await client(`/api/contacts-trash/${props.contactId}/restore`, {
      method: "POST",
    });
    emit("refresh");
...
const handleDelete = async () => {
  ...
  try {
    const client = useSanctumClient();
    const response = await client(`/api/contacts-trash/${props.contactId}`, { method: "DELETE" });
    emit("refresh");
    deleteDialogOpen.value = false;
    toast.success(response.message || "Contact permanently deleted");
  } catch (error) {
