        ...
        <div class="space-y-6">
          <div>
            ...
            <p class="text-muted-foreground mt-1 text-sm tracking-tight">
              Upload an Excel file to import business categories
            </p>
          </div>
            ...
            <div class="flex items-start gap-3">
              ...
              <div class="space-y-2 text-sm tracking-tight">
                ...
                <ul class="text-muted-foreground list-inside list-disc space-y-1">
                  ...
                  <li>Only <strong>Name</strong> column is required</li>
...
const handleDownloadTemplate = async () => {
  try {
    ...
    const response = await sanctumFetch(`${apiBase}/import/template`, {
      responseType: "blob",
    });
    ...
    link.href = url;
    link.download = "business_categories_import_template.xlsx";
...
const handleImport = async () => {
  ...
  try {
    ...
    const response = await sanctumFetch(`${apiBase}/import`, {
      method: "POST",
      body: {
    ...
    if (response.imported_count) {
      parts.push(`${response.imported_count} category(s) imported`);
    }
    if (response.skipped_count) {
      parts.push(`${response.skipped_count} skipped`);
    }

    toast.success(response.message || "Business categories imported successfully", {
    ...
    console.error("Failed to import business categories:", error);
    ...
    if (error?.data?.errors && Array.isArray(error.data.errors)) {
      ...
      toast.error("Failed to import business categories", {
        description: error?.data?.message || error?.message || "An error occurred",
      });
    }
  } finally {
