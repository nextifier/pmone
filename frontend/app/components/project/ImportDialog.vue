  ...
  <ResponsiveDialog v-model:open="isOpen" dialog-max-width="500px" :overflow-content="true">
    ...
    <template #default>
      <div class="px-4 pt-5 pb-8 md:px-6 md:py-5">
        <div class="space-y-6">
          <div>
            <h2 class="text-foreground text-lg font-semibold tracking-tight">Import Projects</h2>
            <p class="text-muted-foreground mt-1 text-sm tracking-tight">
              Upload an Excel file to import multiple projects at once
            </p>
          </div>
...
const handleDownloadTemplate = async () => {
  try {
    ...
    const response = await sanctumFetch("/api/projects/import/template", {
      responseType: "blob",
    });
...
const handleImport = async () => {
  ...
  try {
    ...
    const response = await sanctumFetch("/api/projects/import", {
      method: "POST",
      body: {
