    ...
    <DialogResponsive v-model:open="restoreDialogOpen">
      <template #default>
        <div class="px-4 pb-10 md:px-6 md:py-5">
          <div class="page-title">Restore event?</div>
          <p class="page-description mt-1.5">This will restore this event.</p>
      ...
      <template #default>
        <div class="px-4 pb-10 md:px-6 md:py-5">
          ...
          <p class="page-description mt-1.5">
            This action can't be undone. This will permanently delete this event.
          </p>
          <div class="mt-3 flex justify-end gap-2">
            <button
              class="border-border hover:bg-muted rounded-lg border px-4 py-2 text-sm font-medium tracking-tight active:scale-98"
...
const handleRestore = async () => {
  ...
  try {
    ...
    const response = await client(`/api/events-trash/${props.eventId}/restore`, {
      method: "POST",
    });
    emit("refresh");
...
const handleDelete = async () => {
  ...
  try {
    const client = useSanctumClient();
    const response = await client(`/api/events-trash/${props.eventId}`, { method: "DELETE" });
    emit("refresh");
