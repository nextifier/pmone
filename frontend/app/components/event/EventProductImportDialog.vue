  ...
  <ResponsiveDialog v-model:open="isOpen" dialog-max-width="500px" :overflow-content="true">
    ...
    <template #default>
      <div class="px-4 pt-5 pb-8 md:px-6 md:py-5">
        <div class="space-y-6">
          <div>
            <h2 class="text-foreground text-lg font-semibold tracking-tight">Import Products</h2>
            <p class="text-muted-foreground mt-1 text-sm tracking-tight">
              Upload an Excel file to import multiple products into this event
            </p>
          </div>
              ...
              <div class="space-y-2 text-sm tracking-tight">
                ...
                <ul class="text-muted-foreground list-inside list-disc space-y-1">
                  ...
                  <li>
                    ...
                    Internet & Telecommunication, Accommodation, Marketing
                  </li>
                  <li>Unit options: unit, set, meter, sqm, pcs, kWh (defaults to "unit")</li>
                  <li>
                    Booth Types can be comma-separated: "Raw Space, Standard Shell Scheme"
                  </li>
                  <li>Active can be "Yes" or "No" (defaults to "Yes")</li>
                </ul>
              </div>
            </div>
          </div>
...
const apiBase = computed(
  () => `/api/projects/${props.username}/events/${props.eventSlug}/products`
);
...
const handleDownloadTemplate = async () => {
  try {
    ...
    link.href = url;
    link.download = "event_products_import_template.xlsx";
...
const handleImport = async () => {
  ...
  try {
    ...
    const response = await sanctumFetch(`${apiBase.value}/import`, {
      method: "POST",
      body: {
    ...
    toast.success(response.message || "Products imported successfully", {
      description: response.imported_count
        ? `${response.imported_count} product(s) imported`
        : undefined,
    });
