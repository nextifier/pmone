  ...
  <ResponsiveDialog v-model:open="isOpen" dialog-max-width="500px" :overflow-content="true">
    ...
    <template #default>
      <div class="px-4 pb-10 md:px-6 md:py-5">
        <div class="space-y-6">
          <div>
            <h2 class="text-foreground text-lg font-semibold tracking-tight">Import Partners</h2>
            <p class="text-muted-foreground mt-1 text-sm tracking-tight">
              Upload a spreadsheet to import partners in bulk.
            </p>
          </div>
              ...
              <div class="flex items-start gap-3">
                ...
                <div class="space-y-2 text-sm tracking-tight">
                  ...
                  <ul class="text-muted-foreground list-inside list-disc space-y-1">
                    ...
                    <li>Status defaults to "Active" if empty</li>
                    <li>Visibility defaults to "Public" if empty</li>
                  </ul>
                </div>
              </div>
            </div>
    ...
    if (status === "completed") {
      ...
      if (errorCount > 0) {
        toast.success("Import completed with errors", {
          description: `${p.imported_count} partner(s) imported, ${errorCount} row(s) failed`,
        });
      } else {
        toast.success(p.imported_count
          ? `${p.imported_count} partner(s) imported successfully`
...
const handleDownloadTemplate = async () => {
  try {
    ...
    const response = await sanctumFetch("/api/partners/import/template", {
      responseType: "blob",
    });
    ...
    link.href = url;
    link.download = "partners_import_template.xlsx";
...
const handleImport = async () => {
  ...
  try {
    await startImport("/api/partners/import", {
