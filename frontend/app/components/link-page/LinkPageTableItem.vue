<template>
  <div class="flex flex-col gap-y-0.5 overflow-hidden">
    <div class="flex items-center gap-x-0.5">
      <NuxtLink
        :to="`/link-pages/${linkPage.slug}`"
        class="text-muted-foreground inline text-sm tracking-tight"
      >
        {{ domain }}/<span class="text-foreground font-medium">{{ linkPage.slug }}</span>
      </NuxtLink>
    ...
    <NuxtLink
      :to="`/link-pages/${linkPage.slug}`"
      class="text-foreground line-clamp-1 text-sm font-medium tracking-tight hover:underline"
    >
      {{ linkPage.title }}
    </NuxtLink>
  </div>
...
const publicUrl = computed(() => `${config.public.siteUrl}/${props.linkPage.slug}`);
...
const downloadQR = async (format) => {
  try {
    ...
    if (format === "svg") {
      const svgString = await QRLib.toString(publicUrl.value, {
        type: "svg",
        width: 512,
        margin: 2,
      ...
      a.download = `QR-${props.linkPage.slug}.svg`;
      a.href = url;
      a.click();
      URL.revokeObjectURL(url);
    } else {
      const dataUrl = await QRLib.toDataURL(publicUrl.value, {
        width: 1080,
        margin: 2,
        errorCorrectionLevel: "H",
        color: { dark: "#000000", light: "#FFFFFF" },
      ...
      const a = document.createElement("a");
      a.download = `QR-${props.linkPage.slug}.png`;
      a.href = dataUrl;
      a.click();
    }
