  ...
  <ResponsiveDialog v-model:open="isOpen" dialog-max-width="500px" :overflow-content="true">
    ...
    <template #default>
      <div class="px-4 pb-10 md:px-6 md:py-5">
        <div class="space-y-6">
          <div>
            <h2 class="text-foreground text-lg font-semibold tracking-tight">Import Short Links</h2>
            <p class="text-muted-foreground mt-1 text-sm tracking-tight">
              Upload an Excel file to import multiple short links at once
            </p>
          </div>
            ...
            <div class="flex items-start gap-3">
              ...
              <div class="space-y-2 text-sm tracking-tight">
                ...
                <ul class="text-muted-foreground list-inside list-disc space-y-1">
                  ...
                  <li>Only <strong>Slug</strong> and <strong>Destination URL</strong> are required</li>
                  <li>Supported formats: CSV, XLS, XLSX (max 5MB)</li>
                  <li>Status can be "Active" or "Inactive" (defaults to "Active")</li>
                  <li>Slug must be unique and can only contain letters, numbers, dots, underscores, and hyphens</li>
                  <li>Destination URL must be a valid URL</li>
                </ul>
              </div>
            </div>
          </div>
...
const handleDownloadTemplate = async () => {
  try {
    ...
    const response = await sanctumFetch("/api/short-links/import/template", {
      responseType: "blob",
    });
    ...
    link.href = url;
    link.download = "short_links_import_template.xlsx";
...
const handleImport = async () => {
  ...
  try {
    ...
    const response = await sanctumFetch("/api/short-links/import", {
      method: "POST",
      body: {
    ...
    toast.success(response.message || "Short links imported successfully", {
      description: response.imported_count
        ? `${response.imported_count} short link(s) imported`
        : undefined,
    });
    ...
    console.error("Failed to import short links:", error);
