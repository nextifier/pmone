  ...
  <ResponsiveDialog v-model:open="isOpen" dialog-max-width="500px" :overflow-content="true">
    ...
    <template #default>
      <div class="px-4 pb-10 md:px-6 md:py-5">
        <div class="space-y-6">
          <div>
            <h2 class="text-foreground text-lg font-semibold tracking-tight">Import GA Properties</h2>
            <p class="text-muted-foreground mt-1 text-sm tracking-tight">
              Upload an Excel file to import multiple GA4 properties at once
            </p>
          </div>
          ...
          <div class="flex justify-end gap-2">
            ...
            >
              ...
              <span>Import GA Properties</span>
            </button>
          </div>
        </div>
      </div>
...
const handleDownloadTemplate = async () => {
  try {
    ...
    const response = await sanctumFetch("/api/google-analytics/ga-properties/import/template", {
      responseType: "blob",
    });
    ...
    link.href = url;
    link.download = "ga_properties_import_template.xlsx";
...
const handleImport = async () => {
  ...
  try {
    ...
    const response = await sanctumFetch("/api/google-analytics/ga-properties/import", {
      method: "POST",
      body: {
    ...
    toast.success(response.message || "GA properties imported successfully", {
      description: response.imported_count
        ? `${response.imported_count} propert${response.imported_count === 1 ? "y" : "ies"} imported`
        : undefined,
    });
    ...
    console.error("Failed to import GA properties:", error);
    ...
    if (error?.data?.errors && Array.isArray(error.data.errors)) {
      ...
      toast.error("Failed to import GA properties", {
        description: error?.data?.message || error?.message || "An error occurred",
      });
    }
  } finally {
