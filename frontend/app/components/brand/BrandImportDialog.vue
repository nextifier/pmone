  ...
  <DialogResponsive v-model:open="isOpen" dialog-max-width="500px" :overflow-content="true">
    ...
    <template #default>
      <div class="px-4 pb-10 md:px-6 md:py-5">
        <div class="space-y-6">
          <div>
            <h2 class="text-primary text-lg font-semibold tracking-tight">{{ $t('brandsImport.title') }}</h2>
          ...
          <div class="flex justify-end gap-2">
            ...
            >
              ...
              <span>{{ $t('brandsImport.importBrands') }}</span>
            </button>
          </div>
        </div>
      </div>
...
const handleImport = async () => {
  ...
  try {
    ...
    const response = await sanctumFetch("/api/brands/import", {
      method: "POST",
      body: {
    ...
    toast.success(response.message || "Brands imported successfully", {
      description: response.imported_count
        ? `${response.imported_count} brand(s) imported`
        : undefined,
    });
