...
const handleRestore = async () => {
  ...
  try {
    ...
    const response = await client(`/api/brands-trash/${props.brandId}/restore`, {
      method: "POST",
    });
    emit("refresh");
...
const handleDelete = async () => {
  ...
  try {
    const client = useSanctumClient();
    const response = await client(`/api/brands-trash/${props.brandId}`, { method: "DELETE" });
    emit("refresh");
    deleteDialogOpen.value = false;
    toast.success(response.message || "Brand permanently deleted");
  } catch (error) {
    console.error("Failed to permanently delete brand:", error);
