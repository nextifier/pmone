...
const { options, placeholder, disabled } = defineProps<LocationComboboxProps>();
...
<template>
  <ComboboxRoot
    ...
    :open-on-focus="true"
    :disabled="disabled"
    @update:model-value="(val: any) => { if (val) handleSelect(val) }"
  >
    <ComboboxAnchor class="w-full">
      <div
        class="border-input focus-within:border-ring focus-within:ring-ring has-aria-invalid:ring-destructive/20 dark:has-aria-invalid:ring-destructive/40 has-aria-invalid:border-destructive relative flex h-9 w-full items-center rounded-md border text-sm transition-[color,box-shadow] outline-none focus-within:ring-[1px] has-disabled:pointer-events-none has-disabled:cursor-not-allowed has-disabled:opacity-50"
      >
        <ComboboxInput
          v-model="searchTerm"
          :display-value="() => modelValue || ''"
          :placeholder="placeholder || 'Select...'"
          class="placeholder:text-muted-foreground h-full w-full flex-1 bg-transparent px-3 py-1 text-sm outline-hidden"
        />
