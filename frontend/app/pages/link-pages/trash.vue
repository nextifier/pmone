  ...
  <div class="mx-auto max-w-4xl space-y-6 pt-4 pb-16">
    <div class="flex items-center justify-between gap-x-2.5">
      <div class="flex items-center gap-x-2.5">
        ...
        <h1 class="page-title">Link Pages Trash</h1>
      </div>

      <div class="ml-auto flex shrink-0 gap-1 sm:gap-2">
        <Button variant="outline" size="sm" to="/link-pages">
          <Icon name="hugeicons:link-circle" class="size-4 shrink-0" />
          <span>All Link Pages</span>
        </Button>
      </div>
    </div>
    ...
    <TableData
      ...
      :pending="pending"
      :error="error"
      model="link-pages-trash"
      search-column="title"
      :show-add-button="false"
      search-placeholder="Search title or slug"
      error-title="Error loading trashed link pages"
          ...
          <template #default>
            <div class="px-4 pt-5 pb-8 md:px-6 md:py-5">
              ...
              <p class="text-body mt-1.5 text-sm tracking-tight">
                ...
                {{ selectedRows.length === 1 ? "link page" : "link pages" }}.
              </p>
...
definePageMeta({
  ...
  permissions: ["link_pages.delete"],
  layout: "app",
});

defineOptions({
  name: "link-pages-trash",
});

usePageMeta(null, {
  title: "Link Pages Trash",
});
...
// Fetch trashed link pages with lazy loading
const {
  data: linkPagesResponse,
  pending,
  error,
  refresh: fetchLinkPages,
} = await useLazySanctumFetch(() => `/api/link-pages/trash?${buildQueryParams()}`, {
  key: "link-pages-trash-list",
  watch: false,
});

const data = computed(() => linkPagesResponse.value?.data || []);
const meta = computed(
  () => linkPagesResponse.value?.meta || { current_page: 1, last_page: 1, per_page: 10, total: 0 }
);

const refresh = fetchLinkPages;
...
const columns = [
  ...
  },
  {
    ...
    cell: ({ row }) => {
      return h("div", { class: "space-y-0.5" }, [
        h("div", { class: "text-sm font-medium tracking-tight" }, row.original.title),
        h("div", { class: "text-muted-foreground text-xs sm:text-sm tracking-tight" }, row.original.slug),
      ]);
    },
    size: 300,
    enableHiding: false,
    filterFn: (row, columnId, filterValue) => {
      ...
      return title.includes(searchValue) || slug.includes(searchValue);
    },
  },
  {
    header: "Items",
  ...
  },
  {
    ...
    cell: ({ row }) => {
      const linkPage = row.original;
      return h("div", { class: "flex items-center gap-x-2" }, [
        h(Switch, {
          modelValue: linkPage.is_active,
  ...
  },
  {
    ...
    cell: ({ row }) => h(RowActions, { linkPageId: row.original.id }),
    size: 60,
    enableHiding: false,
  },
];
...
const handleRestoreRows = async (selectedRows) => {
  const linkPageIds = selectedRows.map((row) => row.original.id);
  try {
    restorePending.value = true;
    const client = useSanctumClient();
    const response = await client("/api/link-pages/trash/restore/bulk", {
      method: "POST",
      body: { ids: linkPageIds },
    });
    await refresh();
    ...
    toast.success(response.message || "Link pages restored successfully", {
      description:
        response.errors?.length > 0
          ? `${response.restored_count} restored, ${response.errors.length} failed`
          : `${response.restored_count} link page(s) restored`,
    });
  } catch (error) {
    console.error("Failed to restore link pages:", error);
    toast.error("Failed to restore link pages", {
...
const handleRestoreSingleRow = async (linkPageId) => {
  try {
    restorePending.value = true;
    const client = useSanctumClient();
    const response = await client(`/api/link-pages/trash/${linkPageId}/restore`, {
      method: "POST",
    });
    await refresh();
    ...
    toast.success(response.message || "Link page restored successfully");
  } catch (error) {
    console.error("Failed to restore link page:", error);
    toast.error("Failed to restore link page", {
      description: error?.data?.message || error?.message || "An error occurred",
    });
  } finally {
    restorePending.value = false;
...
const handleDeleteRows = async (selectedRows) => {
  ...
  try {
    ...
    const client = useSanctumClient();
    const response = await client("/api/link-pages/trash/bulk", {
      method: "DELETE",
      body: { ids: linkPageIds },
    });
    await refresh();
    ...
    toast.success(response.message || "Link pages permanently deleted", {
      description:
        response.errors?.length > 0
          ? `${response.deleted_count} deleted, ${response.errors.length} failed`
          : `${response.deleted_count} link page(s) permanently deleted`,
    });
  } catch (error) {
    console.error("Failed to permanently delete link pages:", error);
    toast.error("Failed to permanently delete link pages", {
      description: error?.data?.message || error?.message || "An error occurred",
    });
  } finally {
...
const handleDeleteSingleRow = async (linkPageId) => {
  try {
    deletePending.value = true;
    const client = useSanctumClient();
    const response = await client(`/api/link-pages/trash/${linkPageId}`, { method: "DELETE" });
    await refresh();
    ...
    toast.success(response.message || "Link page permanently deleted");
  } catch (error) {
    console.error("Failed to permanently delete link page:", error);
    toast.error("Failed to permanently delete link page", {
      description: error?.data?.message || error?.message || "An error occurred",
    });
  } finally {
    deletePending.value = false;
...
const RowActions = defineComponent({
  props: {
    linkPageId: { type: Number, required: true },
  },
  setup(props) {
    const restoreDialogOpen = ref(false);
    const deleteDialogOpen = ref(false);
                        ...
                        { asChild: true },
                        {
                          default: () =>
                            h(
                              ...
                              {
                                class:
                                  "hover:bg-destructive/10 text-destructive-foreground rounded-md px-3 py-2 text-left text-sm tracking-tight flex items-center gap-x-1.5",
            ...
            default: () =>
              h("div", { class: "px-4 pt-5 pb-8 md:px-6 md:py-5" }, [
                ...
                h(
                  ...
                  "This will restore this link page."
                ),
                  ...
                  h(
                    "button",
                    {
                      ...
                      onClick: async () => {
                        ...
                        try {
                          await handleRestoreSingleRow(props.linkPageId);
                          restoreDialogOpen.value = false;
                        } finally {
            ...
            default: () =>
              h("div", { class: "px-4 pt-5 pb-8 md:px-6 md:py-5" }, [
                ...
                h(
                  "p",
                  { class: "text-body mt-1.5 text-sm tracking-tight" },
                  "This action can't be undone. This will permanently delete this link page."
                ),
                h("div", { class: "mt-3 flex justify-end gap-2" }, [
                  h(
                    "button",
                  ...
                  h(
                    "button",
                    {
                      ...
                      onClick: async () => {
                        ...
                        try {
                          await handleDeleteSingleRow(props.linkPageId);
                          deleteDialogOpen.value = false;
                        } finally {
