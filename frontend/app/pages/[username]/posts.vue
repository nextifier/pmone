<template>
  <div class="mx-auto space-y-6 pt-4 pb-16 lg:max-w-4xl xl:max-w-6xl">
    ...
    <TableData
      ...
      :columns="columns"
      :meta="meta"
      :pending="pending || userPending || !targetUser"
      :error="targetUser ? (error || userError) : userError"
      model="posts"
      label="Post"
      search-column="title"
...
// Reactive title ref - must be declared before await for usePageMeta
...
const {
  ...
  pending: userPending,
  error: userError,
} = await useLazySanctumFetch(() => `/api/users/${username}`, {
  key: `user-posts-profile-${username}`,
});

const targetUser = computed(() => userResponse.value?.data || null);

// Update page title when user data is loaded
watch(targetUser, (user) => {
  if (user) pageTitle.value = `Posts - ${user.name}`;
}, { immediate: true });
...
const buildQueryParams = () => {
  ...
  if (!targetUser.value?.id) return params.toString();

  params.append("page", pagination.value.pageIndex + 1);
  params.append("per_page", pagination.value.pageSize);
  params.append("filter_creator", targetUser.value.id);
...
const {
  ...
  () => {
    if (!targetUser.value?.id) return null;
    return `/api/posts?${buildQueryParams()}`;
  },
  {
    key: `user-posts-${username}`,
    watch: false,
  },
);
...
// Refetch when user data is loaded
watch(
  () => targetUser.value?.id,
  (id) => {
    if (id) fetchPosts();
...
watch([columnFilters, sorting, pagination], () => fetchPosts(), { deep: true });
