  ...
  <div class="mx-auto max-w-6xl space-y-6 px-4 pt-4 pb-16">
    ...
    <div v-else class="space-y-8">
      <section v-for="group in groupedHotels" :key="group.event.id" class="space-y-4">
        <div class="flex items-end justify-between gap-3 border-b pb-3">
          <div>
            <h2 class="text-lg font-semibold tracking-tighter sm:text-xl">
              {{ group.event.title }}
            </h2>
            <p
        ...
        <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
          <NuxtLink
            ...
            class="bg-card hover:border-foreground/20 group flex flex-col overflow-hidden rounded-xl border tracking-tight transition"
          >
            ...
            <div class="flex flex-1 flex-col gap-1 p-4">
              <h3 class="line-clamp-1 text-base font-semibold tracking-tighter">{{ hotel.name }}</h3>
              <p class="text-muted-foreground line-clamp-1 text-xs tracking-tight sm:text-sm">
                {{ [hotel.address, hotel.city].filter(Boolean).join(", ") }}
              </p>
              <p v-if="cheapestRate(hotel)" class="mt-auto pt-2 text-sm tracking-tight">
                <span class="text-muted-foreground">From</span>
                <span class="mx-1 font-medium tabular-nums">
...
const { data, pending } = await useLazyAsyncData("public-hotels", () => $fetch("/api/hotels"));
