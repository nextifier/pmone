  ...
  <div class="container pt-6 pb-24">
    ...
    <div class="flex flex-col gap-y-4">
      ...
      >
        ...
        <div class="flex -space-x-px sm:gap-x-1.5 sm:space-x-0">
          <div
            v-for="shade in shades"
            :key="shade"
            class="group relative w-full cursor-pointer flex-col items-center space-y-1.5 text-center"
            @click="copyColor(palette.name, shade, palette.colors[shade])"
          >
            <div
              class="outline-primary/10 h-10 w-full outline -outline-offset-1 sm:rounded-md"
              :style="{ backgroundColor: palette.colors[shade] }"
            />
            <div
              class="text-muted-foreground hidden font-mono text-xs font-medium tracking-tight sm:block"
            >
              {{ shade }}
            </div>
            <!-- Hover tooltip -->
...
const selectedFormat = useLocalStorage<ColorFormat>("color-format", "hex");

// --- Color conversion (client-side only via canvas) ---
