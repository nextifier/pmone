<template>
  <div class="mx-auto max-w-xl space-y-9">
    ...
    <form v-else-if="shortLink" @submit.prevent="handleSubmit" class="space-y-6">
      <div class="space-y-2">
      ...
      <div class="border-border space-y-2 rounded-lg border p-4">
        <div class="text-muted-foreground text-xs">Statistics</div>
        <div class="text-sm">
          <strong>Total Clicks:</strong> {{ shortLink.clicks_count?.toLocaleString() || 0 }}
        </div>
        <nuxt-link
          :to="`/short-links/${shortLink.slug}/analytics`"
          class="text-primary hover:underline inline-flex items-center gap-x-1 text-sm"
        >
...
async function loadShortLink() {
  try {
    ...
    // Populate form
    form.value = {
      slug: shortLink.value.slug,
      destination_url: shortLink.value.destination_url,
      is_active: shortLink.value.is_active,
    };
  } catch (err) {
...
async function handleSubmit() {
  ...
  try {
    ...
    if (err.response?.status === 422 && err.response?._data?.errors) {
      ...
      const errorMessage =
        err.response?._data?.message || err.message || "Failed to update short link";
