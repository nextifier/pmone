  ...
  <div class="mx-auto max-w-6xl space-y-6 pt-4 pb-16">
    <div class="flex flex-wrap items-center justify-between gap-x-2.5 gap-y-4">
      <div class="flex shrink-0 items-center gap-x-2.5">
        ...
        <h1 class="page-title">
          Posts by
          <span v-if="targetUser" class="text-primary">{{ targetUser.name }}</span>
        </h1>
      </div>
    </div>
    ...
    >
      ...
      <template #actions="{ selectedRows }">
        <DialogResponsive
          v-if="selectedRows.length > 0 && canManagePosts"
...
const username = computed(() => route.params.slug);
...
const fetchTargetUser = async () => {
  try {
    ...
    const response = await client(`/api/users/${username.value}`);
...
const columns = [
  ...
  },
  {
    ...
    cell: ({ row }) => h(RowActions, { post: row.original, canManage: canManagePost(row.original) }),
    size: 60,
    enableHiding: false,
  },
];
...
// Row Actions Component with role checking
  ...
  setup(props) {
    ...
    return () =>
      h("div", { class: "flex justify-end" }, [
                  ...
                  default: () =>
                    h(
                      ...
                      [
                        ...
                        // Only show Edit button if user can manage
                        ...(props.canManage
                          ? [
                              h(
                                PopoverClose,
        ...
        // Only show delete dialog if user can manage
        ...(props.canManage
          ? [
              h(
                DialogResponsive,
