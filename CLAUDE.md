...
- Frontend (Nuxt 4) is located in the `/frontend` folder within the Laravel root directory.

## Frontend (Nuxt 4)
- Frontend application is built with Nuxt 4 and located in `/frontend` directory.
- To run frontend development: `cd frontend && npm run dev`
- To build frontend: `cd frontend && npm run build`
- Frontend will be deployed separately to Cloudflare Pages (pmone.id).
- Backend API will be deployed to DigitalOcean via Laravel Forge (api.pmone.id).

## Frontend Bundling
- If the user doesn't see a frontend change reflected in the UI, it could mean they need to run `npm run build`, `npm run dev`, or `composer run dev`. Ask them.
- For frontend changes: run `cd frontend && npm run dev` or `cd frontend && npm run build`.
